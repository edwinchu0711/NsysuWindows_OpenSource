...
class _ScoreResultPageState extends State<ScoreResultPage> {
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      ...
                                      child: SingleChildScrollView(
                                        ...
                                        child: Column(
                                          children: [
                                            _buildDesktopSelectorArea(sortedYears, availableSems),
                                            const SizedBox(height: 16),
                                            _buildScoreSummaryPane(_selectedYear!, _selectedSem!),
                                          ],
                                        ),
                                      ),
                                    ),
                                    ...
                                    // 右側：課程成績 (Expanded)
                                    Expanded(
                                      child: _buildCourseListPane(_selectedYear!, _selectedSem!),
                                    ),
                                  ],
                                ),
                              ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDropdown("選擇學期", availableSems, _selectedSem!, (val) {
  ...
  bool _isRankPreviewOffPeriod() {
    ...
    // 期間 1: 3/20 ~ 6/5
    bool inPeriod1 = (month == 3 && day >= 20) || (month > 3 && month < 6) || (month == 6 && day <= 5);
    // 期間 2: 10/15 ~ 1/5 (跨年)
  ...
  Widget _buildPersistentInfoBox() {
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (context, snapshot) {
        ...
        bool isPreviewEnabled = snapshot.data!.getBool('is_preview_rank_enabled') ?? false;
        ...
        String description = "目前沒有開啟預覽名次，抓取速度快。若是要看預覽名次，請到設定頁面去開啟。\n※ 注意：3/20~6/5 和 10/15~1/5 期間此功能會強制關閉，為了節省時間。";
        if (isCurrentlyActive) {
          description = "現在有開啟預覽名次，每次抓取會比較久。若是已抓到要的資料，可以先去設定關閉以加快速度。\n※ 注意：1. 3/20~6/5 和 10/15~1/5 期間此功能會強制關閉。 2. 若寒暑假期間查無預覽資料，推測為系統更動，建議暫時關閉以維持效率。";
        } else if (isEnabledButInactive) {
          description = "您已開啟預覽名次功能，但目前正處於系統自動關閉期間 (3/20~6/5 或 10/15~1/5)，因此目前不會有預覽效果且不影響抓取速度。\n待限制期間過後，功能將自動恢復運作。";
        }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      description,
                      style: TextStyle(color: titleColor.withOpacity(0.85), fontSize: 12, height: 1.4),
                    ),
                  ],
                ),
              ),
  ...
  Widget _buildScoreSummaryPane(String year, String sem) {
    ...
    bool hasValidValue(String? value) => value != null && value.isNotEmpty && value != "-";
    ...
    if (isOfficialValid) {
      ...
      if (!hasValidValue(finalSummary.rank) && previewData != null && hasValidValue(previewData['rank'])) {
  ...
  Widget _buildCourseCard(CourseScore course) {
    ...
    return Container(
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 12.0), // 縮減內部上下間距
        child: Row(
          children: [
            Container(
            ...
            Text(course.score, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: scoreColor)),
          ],
        ),
      ),
    );
  ...
  Widget _buildSummaryCard(ScoreSummary summary, SummaryType type) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12), // 稍微縮減上下內距
      decoration: BoxDecoration(
          ...
          SizedBox(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(title, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: themeColor)),
                if (type == SummaryType.preview) ...[
                  ...
                  GestureDetector(
                    ...
                    child: Icon(Icons.info_outline_rounded, color: themeColor.withOpacity(0.7), size: 16),
                  )
                ]
              ],
            ),
          ),
          Divider(color: themeColor.withOpacity(0.2), height: 12), // 縮小 Divider 的高度
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween, 
            children: [
